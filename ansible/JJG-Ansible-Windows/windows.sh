...
# windows-ansible-ansible: https://github.com/KSid/windows-ansible-ansible
#
# @see README.md
...
if [ -f ./ansible/requirements.txt ]; then
  sudo ansible-galaxy install -r ./ansible/requirements.txt
...
ansible-playbook -i 'localhost,' ./ansible/${ANSIBLE_PLAYBOOK} --extra-vars "is_windows=true" --connection=local
